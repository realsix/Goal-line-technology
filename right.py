...
class RightPost:
    ...
    def roi_mask(self, gray_img, right=True):
        """
        对gray_img进行掩膜
        :param gray_img: 灰度图,channels=1
        """
        if right:
            poly_pts = np.array([[[165,240],[165,0], [320,0], [320, 240]]])
        else:
            poly_pts = np.array([[[125,240], [125,0], [211, 0], [211, 240]]])
    ...
    def cluster(self, lines, right=True):
        this = [lines[0]]
        another = []
        for i in range(1, len(lines)):
            slope = self.calculate_slope([lines[i][0]])
            if slope > 10 if right else slope < 10:
...
if __name__ == '__main__':
    cap = cv2.VideoCapture('right.avi')
    ...
    while True:
        ...
        rightpost.show_img(rightpost.color_img)
        print(rightpost.inner)
